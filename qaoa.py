...
from _quantum_computer import _measure_bitstrings
...
#            print('Prog',prog)
#            print(isinstance(prog, pq.Program))
...
#        print('sampling_prog belongs to Program', isinstance(sampling_prog, pq.Program))
#        bitstring_samples = self.qvm.run_and_measure(sampling_prog,
#                                                     self.qubits,
#                                                     trials=samples)

        qc = self.qvm
        bitstring_samples = _measure_bitstrings(qc, sampling_prog, self.qubits, samples)

        bitstring_tuples = list(map(tuple, bitstring_samples))
#        print('Bitstring tuples1', bitstring_tuples[0])
#        print('Bitstring tuples2', bitstring_tuples[1])
#        print('Length:', len(bitstring_tuples))
        bitstring_tuples_arr = []
        
        for x in bitstring_tuples:
            for y in x:
                bitstring_tuples_arr.append(str(y))
        
        freq = Counter(bitstring_tuples_arr)
        print('frequency:', freq.values())
        most_frequent_bit_string = max(freq, key=lambda x: freq[x])
#        print('Most frequent string:', most_frequent_bit_string)
